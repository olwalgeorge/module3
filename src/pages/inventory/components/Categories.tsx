  ...
  const getCategoryStats = (categoryName: string) => {
    ...
    categoryProducts.forEach(product => {
      ...
      productLocations.forEach(location => {
        ...
        if (warehouse) {
          const current = warehouseDistribution.get(warehouse.name) || 0;
          warehouseDistribution.set(warehouse.name, current + location.currentStock);
        }
      });
    });
    ...
    return {
      ...
      totalValue,
      lowStockItems,
      warehouseDistribution: Array.from(warehouseDistribution.entries())
    };
  };
